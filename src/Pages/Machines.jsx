        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          ...
          <div className="relative w-full max-w-3xl bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden animate-pop">
            ...
            <button
              ...
              className="absolute top-3 sm:top-4 right-3 sm:right-4 w-9 sm:w-10 h-9 sm:h-10 rounded-full bg-gray-100 hover:bg-red-100 text-gray-600 hover:text-red-600"
            >
              ✕
            </button>

            {/* IMAGE */}
            <div className="bg-[#f7f9fc] flex items-center justify-center p-4 sm:p-6">
              <img
                src={selected.img}
                alt={selected.title}
                className="h-[200px] sm:h-[250px] md:h-[280px] object-contain hover:scale-105 transition duration-500"
              />
            </div>

            {/* CONTENT */}
            <div className="p-5 sm:p-8 text-center sm:text-left">

              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#1f2d6b]">
                {selected.title}
              </h2>

              <div className="w-14 sm:w-16 h-1 bg-[#1596d4] rounded-full my-3 mx-auto sm:mx-0"></div>
              ...
              <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-5">
                <span className="px-3 py-1 text-xs bg-blue-50 rounded-full text-[#1f2d6b]">
                  Medical Grade
                </span>
                <span className="px-3 py-1 text-xs bg-blue-50 rounded-full text-[#1f2d6b]">
                  FDA Approved
